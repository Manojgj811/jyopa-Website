...
function initializeVisia() {
	...
	//PARALLAX EFFECTS
	jQuery('.parallax-bg1').parallax("50%", 0);
	jQuery('.parallax-bg2').parallax("50%", 0.5);
	jQuery('.parallax-bg3').parallax("500%", 10);
		...
		hash = jQuery(window.location).attr('hash'); 
		 ...
		 if( hash.substr(0,rootLength) != root ){
			...
			/* CLICKING ON PORTFOLIO GRID OR THROUGH PROJECT NAVIGATION */	
				...
				jQuery('html,body').delay(800).stop().animate({scrollTop: (projectContainer.offset().top-headerH)+'px'},800,'easeOutExpo', function(){ 		
