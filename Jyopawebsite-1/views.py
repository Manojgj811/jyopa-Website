...
def getcandidatedetails(request):
    ...
    try:
        ...
        au.authenticate(request.headers.get('Authorization'),'user')
    except Exception as ex:
        print(ex)
    ...
    try:
        eid = au.get_emp_id_from_token(request.headers.get('Authorization'))
        print("eid")
    except Exception as ex:
        print(ex)
...
def applytojob(request):
    if request.method == 'POST':
        ...
        try:
            send_db['candidateId'] = au.get_emp_id_from_token(request.headers.get('Authorization'))
        except Exception as ex:
            print(ex)
            response = ar.db_error()
            return HttpResponse(content = json.dumps(response),status=500)
